...
export function Jenkins() {
  ...
  // Add a new function to fetch job info
const fetchJobInfo = async (jobName) => {
  try {
    setLoading(prev => ({ ...prev, details: true }));
    const response = await fetch(
      `${baseUrl}/job/${jobName}/api/json?depth=1`,
      { headers: getHeaders() }
    );
  ...
  const getJobStatusFromLastBuild = (lastBuild) => {
    if (!lastBuild) return { 
      text: 'Unknown', 
...
<Modal 
  ...
  title={`Job Information: ${jobInfo?.name || ''}`}
>
  {loading.details ? (
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            <span className="text-gray-600">Concurrent Build:</span>
            <span className="ml-2">{jobInfo.concurrentBuild ? 'Yes' : 'No'}</span>
          </div>
          <div>
            <span className="text-gray-600">Job Type:</span>
            <span className="ml-2">{jobInfo._class.split('.')?.pop()}</span>
          </div>
        </div>
      </div>
      ...
      <div>
        <h3 className="font-semibold">Last Build</h3>
        {jobInfo.lastBuild ? (
          <div className="grid grid-cols-2 gap-2">
            <div>
              <span className="text-gray-600">Number:</span>
              <span className="ml-2">{jobInfo.lastBuild.number}</span>
            </div>
            <div>
              <span className="text-gray-600">URL:</span>
              <a 
                href={jobInfo.lastBuild.url} 
                target="_blank" 
      ...
      {jobInfo.scm && (
        <div>
          <h3 className="font-semibold">Source Code Management</h3>
          <div>
            <span className="text-gray-600">Repository:</span>
            <span className="ml-2">{jobInfo.scm.userRemoteConfigs?.[0]?.url || 'N/A'}</span>
          </div>
        </div>
      )}
